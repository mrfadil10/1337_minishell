...
/*   Updated: 2023/08/11 21:00:47 by mfadil           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_data	*new_data(t_data *prev, char *str, int t_type)
{
	...
	new_data = ft_malloc(sizeof(t_data));
	new_data->t_type = t_type;
	if (t_type == SIQUOTE || t_type == DOQUOTE)
	{
		new_data->av = ft_malloc(sizeof(char *) * 2);
		new_data->av[0] = type_strdup(str);
		new_data->av[1] = 0;
	}
	else
		new_data->av = ft_mysplit(str, ' ');
	new_data->prev = prev;
	new_data->next = NULL;
	return (new_data);
}
